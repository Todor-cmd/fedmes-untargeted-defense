...
def load_ba_data_loader(logger, args):
    """
    Loads the training data DataLoader object from a file if available.
    ...
    :param args: Arguments
    """
    if os.path.exists(args.get_ba_data_loader_pickle_path()):
        return load_data_loader_from_file(logger, args.get_ba_data_loader_pickle_path())
    else:
        logger.error("Couldn't find train data loader stored in file")
...
def load_dba_data_loader(logger, args):
    ...
    :param args: Arguments
    """
    if os.path.exists(args.get_dba_data_loader_pickle_path()):
...
def generate_ba_loader(args, dataset):
    benign_dataset = dataset.get_ba_dataset()
    X, Y = shuffle_data(args, benign_dataset)

    return dataset.get_data_loader_from_data(args.get_batch_size(), X, Y)

def generate_dba_loader(args, dataset):
    malicious_dataset = dataset.get_dba_dataset()
...
def generate_backdoor_test_loader(args, dataset):
    test_dataset = dataset.get_backdoor_test_dataset()
    X, Y = shuffle_data(args, test_dataset)
