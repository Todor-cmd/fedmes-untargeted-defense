...
def distribute_batches_iid_ba(train_data_loader, ba_data_loader, mal_prop, num_workers, args):
    ...
    if mal_prop == 0:
        return distribute_batches_equally(train_data_loader, num_workers)
    elif mal_prop > 0 and mal_prop <= 1:
        distributed_dataset = [[] for i in range(num_workers)]
...
def distribute_batches_iid_dba(train_data_loader, dba_data_loader, mal_prop, num_workers, args):
    args.get_logger().info("Distribute data for iid dba")
    ...
    if mal_prop == 0:
        ...
        for batch_idx, (data, target) in enumerate(dba_data_loader):
            worker_idx = batch_idx % num_workers
            if worker_idx >= num_workers * (1 - mal_prop):
                distributed_dataset[worker_idx].append((data, target))
            else:
